...
- **Catalogue**: `./catalog.js` defines global `HD_CATALOG`
  ...
  Built into `CATALOG = [{key, cat, group, name, sell, barcode}]` where
  **`key = cat + '||' + name`** — this exact key joins every store
  (buy list, order lines, tier price lookups).
  `group` (row field `g`, display-only, NEVER part of the key) is the
  Woolworths-style aisle from GROUP_MAP in build-catalog.cjs: produce cats
  (A-B…S-Z, Herbs) keep group === cat; grocery cats map to Dairy Eggs &
  Fridge / Pantry / Snacks & Confectionery / Drinks / Freezer / Household &
  Personal Care. UI: chip rail = `groups()` (aisles), section headers =
  `orderedCats(group)` (fine categories, aisle-ordered).
