...
// ---- HARD LEAK GUARD: refuse to publish anything but {c,n,s,b,g} ----
// The build FAILS (no file written) if a row carries any other key or a
// suspicious extra numeric column — purchase costs can never ship again.
const ALLOWED_KEYS = new Set(['c', 'n', 's', 'b', 'g']);
for (const r of out) {
  for (const k of Object.keys(r)) {
    if (!ALLOWED_KEYS.has(k)) {
      console.error('LEAK GUARD: row "' + r.n + '" carries forbidden key "' + k + '" — build aborted.');
      process.exit(1);
    }
  }
  if (r.s !== null && typeof r.s !== 'number') {
    console.error('LEAK GUARD: row "' + r.n + '" has a non-numeric sell — build aborted.');
    process.exit(1);
  }
}
...
// IMPORTANT: app2/catalog.js (OUT) is NO LONGER written here. The v2 customer
// app now MIRRORS the printed order form and is generated by
// inv-work/gen_catalog.py — writing app2 from this builder would silently
// un-mirror v2. This builder writes ONLY app3 (the in-house app, which keeps
// the shopProducts produce buckets + cost/margin overlay).
void OUT;   // retained for reference; intentionally not written
fs.writeFileSync(OUT_V3, banner + body, 'utf8');
console.log('wrote ' + OUT_V3 + ' (app3 in-house only — app2 is order-form-mirrored via gen_catalog.py) — ' +
  out.length + ' products, cost-free (' + (PP.addEach || []).length + ' per-piece units merged).');
